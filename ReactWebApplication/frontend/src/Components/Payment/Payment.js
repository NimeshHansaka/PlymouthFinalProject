...
const Payment = ({ totalBill, referenceNumber, clientId, phoneNumber }) => {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            checkPaymentStatus();
        }, 3000); // Polling interval: check every 5 seconds
    ...
    return (
        <PayPalScriptProvider options={{ 'client-id': 'AWG795VHTb_d1tsRKfMZ4_-K2J93C5fy_pTV5Vgz99igeQJTQXgcZOuosUyqoKDkydCllw1R4uf95tPv' }}>
            <div className="payment-information-container">
                <h2 className="payment-title" style={{ color: 'black' }}>Payment Information</h2>
                <p className="total-bill" style={{ color: 'black' }}>Total Bill: ${totalBill}</p>
                <p className="reference-number" style={{ color: 'black' }}>Reference Number: {referenceNumber}</p>
                <PayPalButtons
                ...
                <div className="qr-code-container">
                    <h3 style={{ color: 'black' }}>Scan QR Code to Make Payment</h3>
